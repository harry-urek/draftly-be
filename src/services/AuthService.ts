...
export class AuthService {
  ...
  async registerOrLoginUser(firebaseUser: FirebaseUser): Promise<ServiceResult<{
    user: User;
    ...
    try {
      ...
      let user = await this.userRepository.findByFirebaseUid(firebaseUser.firebaseUid);
      ...
      const tokens = await this.userRepository.getTokens(firebaseUser.firebaseUid);
      ...
      const needsOnboarding = user.onboardingStatus === 'NOT_STARTED' || 
  ...
  async getAuthStatus(firebaseUid: string): Promise<ServiceResult<{
    user: User;
  ...
  async handleGmailCallback(code: string, firebaseUid: string): Promise<ServiceResult<void>> {
    try {
      ...
      await this.userRepository.updateOnboardingStatus(firebaseUid, 'GMAIL_CONNECTED');
