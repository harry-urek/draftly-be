...
export class AuthControllerImpl implements AuthController {
  ...
  registerRoutes(fastify: FastifyInstance): void {
    ...
    fastify.post("/auth/register", { preHandler: [requireAuth()] }, async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const { user: userProfile, needsGmailAuth, needsOnboarding } = result.data!;

        const status = userProfile.onboardingStatus === 'ACTIVE' ? 'authenticated' : 'registered';
    ...
    fastify.get("/auth/status", { preHandler: [requireAuth()] }, async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        const user = (request as any).firebaseUser!;
        ...
        const { user: profile, hasValidGmailAuth, needsOnboarding } = result.data!;
    ...
    fastify.get("/auth/oauth/google/start", async (request: FastifyRequest, reply: FastifyReply) => {
      try {
    ...
    fastify.get("/auth/oauth/google/callback", async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const result = await this.authService.handleGmailCallback(code, state);

        if (!result.success) {
          const frontendUrl = config.frontendUrl;
          return reply.redirect(`${frontendUrl}?error=oauth_failed&message=${encodeURIComponent(result.error || 'OAuth failed')}`);
        }
